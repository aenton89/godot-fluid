...
func build_grid():
	grid.clear()
	for p in particles:
		var cell = get_cell(p.x)
		if not grid.has(cell):
			grid[cell] = []
		grid[cell].append(p)

func get_neighbours(p: FluidParticle) -> Array:
	var cell = get_cell(p.x)
	var neighbours = []
...
func compute_density_pressure():
	...
	for pi in particles:
		pi.rho = 0.0
		var neighbours = get_neighbours(pi)
...
func compute_forces():
	for pi in particles:
		...
		for pj in neighbours:
			...
			if r < H and r > 0:
				var rij_norm = rij / r
				fpress += -rij_norm * MASS * (pi.p + pj.p) / (2.0 * pj.rho) * SPIKY_GRAD * pow(H - r, 3.0)
...
func _on_window_spatial_hash_size_changed() -> void:
